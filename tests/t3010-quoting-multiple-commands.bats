...
@test "--piped --command {} --command {}" {
    ...
    [ "${lines[0]}" = "$commandSingleQuoted $barEscaped ; $commandTwoSingleQuoted $barEscaped < $barEscaped" ]
    assert_modifications
}
...
@test "--piped --command {} -- simple {} --" {
    ...
    [ "${lines[0]}" = "$commandSingleQuoted $barEscaped ; $commandTwoEscaped $barEscaped < $barEscaped" ]
    assert_modifications
}
